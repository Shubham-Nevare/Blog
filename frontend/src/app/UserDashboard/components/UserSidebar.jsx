const menuItems = [
  ...
  { key: "publish", label: "Published Blogs" },
  { key: "pending", label: "Pending Blogs" },
  { key: "reject", label: "Rejected Blogs" },
//   { key: "manage", label: "Manage Blogs" },
];

const UserSidebar = ({ selected, onSelect, blogCount, onLogout }) => (
  <div className="w-56 h-screen bg-gray-900 text-white flex flex-col justify-between fixed left-0 top-0 shadow-lg">
    <div>
      <div className="py-8 px-6 border-b border-gray-800">
        <h2 className="text-xl font-bold tracking-wide text-white">User Dashboard</h2>
      </div>
      <ul className="mt-8">
        {menuItems.map(item => (
          <li
            key={item.key}
            className={`px-6 py-3 cursor-pointer text-base transition-colors ${selected === item.key ? 'bg-blue-700 font-semibold' : 'hover:bg-gray-800'}`}
